...
function seedDb() {
  ...
  testProjects.push(db.insert({name: 'test project 1', date: '2014-12-12', year: 2014, month: 12, day: 12}, 'project'));
  testProjects.push(db.insert({name: 'test project 2', date: '1928-01-06', year: 1928, month: 01, day: 06}, 'project'));
  testProjects.push(db.insert({name: 'test project 3', date: '2002-08-30', year: 2002, month: 08, day: 30}, 'project'));
  ...
  it('should add a new liked project to the user', (done) => {
    ...
    request(app)
      ...
      .end((error, response) => {
        ...
        expect(db.getCollection('user').get(testUsers[0].id).likes.length).toBe(1);
        done();
      });
  });
  ...
  it('should add additional liked projects to the user', (done) => {
    ...
    request(app)
      ...
      .end((error, response) => {
        ...
        expect(db.getCollection('user').get(testUsers[0].id).likes.length).toBe(2);
        done();
      });
  });
  ...
  it('should remove a liked project to the user', (done) => {
    ...
    request(app)
      ...
      .end((error, response) => {
        ...
        expect(db.getCollection('user').get(testUsers[0].id).likes.length).toBe(0);
        done();
      });
  });
